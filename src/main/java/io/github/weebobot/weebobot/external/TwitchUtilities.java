...
public class TwitchUtilities {
	...
	public static void init(String clientID) {
		TwitchUtilities.clientID = "?client_id=" + clientID;
	}
        ...
        try {
            return new JsonParser().parse(
                    new JsonReader(new InputStreamReader(new URL(
                            BASE_URL + "channels/" + channelNoHash + clientID).openStream()))).getAsJsonObject().getAsJsonPrimitive("status").getAsString();
        } catch (IOException e) {
    ...
    public static String getGame(String channelNoHash) {
        try {
            return new JsonParser().parse(
                    new JsonReader(new InputStreamReader(new URL(
                            BASE_URL + "channels/" + channelNoHash + clientID).openStream()))).getAsJsonObject().getAsJsonPrimitive("game").getAsString();
        } catch (IOException e) {
	...
	public static boolean isFollower(String channelNoHash, String sender) {
		try {
			...
			JsonObject obj = new JsonParser().parse(
					new JsonReader(new InputStreamReader(new URL(nextUrl + clientID)
	...
	public static int followerCount(String channelNoHash) {
		try(InputStream stream = new URL(
                BASE_URL + "channels/" + channelNoHash + "/follows" + clientID)
                .openStream()) {
	...
	public static boolean isLive(String channelNoHash) {
		try(InputStream stream = new URL(BASE_URL + "streams/" + channelNoHash + clientID).openStream()) {
			new JsonParser()
